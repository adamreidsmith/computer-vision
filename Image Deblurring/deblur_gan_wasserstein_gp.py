...
Originally trained on the GOPRO dataset.  Training is based on the Wasserstein GAN algorithm with 
gradient penalty.
'''
...
def gradient_penalty(discriminator, real_imgs, gen_imgs, device, gamma=10):
    batch_size = real_imgs.size(0)
    epsilon = torch.rand(batch_size, 1, 1, 1)
    epsilon = epsilon.expand_as(real_imgs)
    epsilon = epsilon.to(device)

    interpolation = epsilon * real_imgs.data + (1 - epsilon) * gen_imgs.data
    ...
    return torch.mean(gamma * ((gradients_norm - 1) ** 2)).to(device)
    ...
    try:
        ...
        for epoch in range(n_epochs):
            ...
            for i, imgs in enumerate(train_loader):
                ...
                if verbose: print('Computing gradient penalty...')
                grad_penalty = gradient_penalty(D, sharp, gen_imgs, device)

                # Adversarial loss + penalty
                if verbose: print('Training discriminator...')
                D_loss = -torch.mean(D(sharp)) + torch.mean(D(gen_imgs)) + grad_penalty
